
const Articals = () => {
    return (
        <div className=" container mx-auto">
            <h2 className="text-[36px] font-semibold">All Articles </h2>
            <div className="flex w-[90%] mb-8 md:w-[60%]">
            <h2 className=" ">We share common trends, strategies ideas, opinions, short & long stories from the team behind company.</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 ">
            <div className=" text-white bg-cover bg-no-repeat rounded  bg-base-100 image-full  shadow-xl" style={{
    backgroundImage: "url(https://i.ibb.co.com/q7mdYnX/Image-4.png)",
  }}>

                 <div className=" p-[5%]">
                 <p className=" badge bg-white text-[18px] text-green-500  font-semibold">Feature</p>
                    <div className=" pt-[65%]">

                    <h2 className=" text-[32px]  font-semibold">How to prevent and protect your family from carbon monoxide</h2>
                    <div className=" flex justify-between mt-4 lg:mt-12 ">
        <div className=" flex gap-4">
        <div className="avatar">
  <div className="w-14 rounded-full">
    <img src="https://i.ibb.co.com/1RQH0B2/download-1.jpg" />
  </div>
         </div>
         <div>
            <h2 className=" text-[20px] font-mono font-semibold">Lina Hicks</h2>
            <h2 className="badge badge-info">Verified</h2>
         </div>
        </div>
        <div>
            <p className=" font-thin">4:55 PM</p>
            <p className=" font-thin">02 May</p>
        </div>
     </div>
                    </div> 
                </div>
           </div>
            <div className=" text-white bg-cover bg-no-repeat rounded  bg-base-100 image-full  shadow-xl" style={{
    backgroundImage: "url(https://i.ibb.co.com/BLznrP5/Image-3.png)",
  }}>

                 <div className=" p-[5%]">
                 <p className=" badge bg-white text-[18px] text-green-500  font-semibold">Feature</p>
                    <div className=" pt-[65%]">

                    <h2 className=" text-[36px] shadow-md font-semibold">Motherhood is the hardest and the best job ever</h2>
                    <div className=" flex justify-between mt-4 lg:mt-12 ">
        <div className=" flex gap-4">
        <div className="avatar">
  <div className="w-14 rounded-full">
    <img src="https://i.ibb.co.com/TLNPGwF/generative-ai-young-smiling-man-avatar-man-brown-beard-mustache-hair-wearing-yellow-sweater-sweatshi.webp" />
  </div>
         </div>
         <div>
            <h2 className=" text-[20px] font-mono font-semibold">Tyler Murray</h2>
            <h2 className="badge badge-info">Verified</h2>
         </div>
        </div>
        <div>
            <p className=" font-thin">4:55 PM</p>
            <p className=" font-thin">02 May</p>
        </div>
     </div>
                    </div> 
                </div>
           </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 mb-12 lg:grid-cols-3 gap-6 ">
           <div className=" text-white bg-no-repeat rounded  bg-base-100 image-full bg-cover  shadow-xl" style={{
    backgroundImage: "url(https://i.ibb.co.com/NYRrLm5/Base.png)",
  }}>

                 <div className="pt-[70%] p-[5%]">
                    <h2 className=" text-[28px] font-bold">Future Of Work</h2>
                    <h2 className=" text-[20px]">Majority of peole will work in jobs that don’t exist today.</h2>
                    <div className=" flex justify-between mt-4 lg:mt-12 ">
        <div className=" flex gap-4">
        <div className="avatar">
  <div className="w-14 rounded-full">
    <img src="https://img.daisyui.com/images/stock/photo-1534528741775-53994a69daeb.webp" />
  </div>
         </div>
         <div>
            <h2 className=" text-[20px] font-mono font-semibold">Miss Diana</h2>
            <h2 className="badge badge-info">Verified</h2>
         </div>
        </div>
        <div>
            <p className=" font-thin">4:55 PM</p>
            <p className=" font-thin">02 May</p>
        </div>
     </div>
                </div>
           </div>
           <div className=" text-white bg-cover bg-no-repeat rounded  bg-base-100 image-full  shadow-xl" style={{
    backgroundImage: "url(https://i.ibb.co.com/XCDhmyr/Base-1.png)",
  }}>

                 <div className="pt-[70%] p-[5%]">
                    <h2 className=" text-[28px] font-bold">Future Of Data</h2>
                    <h2 className=" text-[20px]">Thanks to never-ending piles of data & the amount of insight</h2>
                    <div className=" flex justify-between mt-4 lg:mt-12 ">
        <div className=" flex gap-4">
        <div className="avatar">
  <div className="w-14 rounded-full">
    <img src="https://img.daisyui.com/images/stock/photo-1534528741775-53994a69daeb.webp" />
  </div>
         </div>
         <div>
            <h2 className=" text-[20px] font-mono font-semibold">Miss Diana</h2>
            <h2 className="badge badge-info">Verified</h2>
         </div>
        </div>
        <div>
            <p className=" font-thin">4:55 PM</p>
            <p className=" font-thin">02 May</p>
        </div>
     </div>
                </div>
           </div>
           <div className=" text-white bg-cover bg-no-repeat rounded  bg-base-100 image-full  shadow-xl" style={{
    backgroundImage: "url(https://i.ibb.co.com/vZXfw7B/Base-2.png)",
  }}>

                 <div className="pt-[70%] p-[5%]">
                    <h2 className=" text-[28px] font-bold">Future Of Learning</h2>
                    <h2 className=" text-[20px]">A constant ability to learn will be on the most crucial skills</h2>
                    <div className=" flex justify-between mt-4 lg:mt-12 ">
        <div className=" flex gap-4">
        <div className="avatar">
  <div className="w-14 rounded-full">
    <img src="https://img.daisyui.com/images/stock/photo-1534528741775-53994a69daeb.webp" />
  </div>
         </div>
         <div>
            <h2 className=" text-[20px] font-mono font-semibold">Miss Diana</h2>
            <h2 className="badge badge-info">Verified</h2>
         </div>
        </div>
        <div>
            <p className=" font-thin">4:55 PM</p>
            <p className=" font-thin">02 May</p>
        </div>
     </div>
                </div>
           </div>
        </div>
            
        </div>
    );
};

export default Articals;